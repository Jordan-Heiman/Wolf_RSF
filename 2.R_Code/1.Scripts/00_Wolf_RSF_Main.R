...
# landcover should not have been in the model building with all the continuous models
# for the used/available points need to add column to distinguish different hab classes

att_tbl <- data.frame(id = 0:16, 
                      habitat_type = c(NA, "Open Conifer", "Moderate Conifer", 
                                       "Closed Conifer", "Deciduous", "Mixed Forest", 
                                       "Regeneration", "Herbaceous", "Shrub", "Water",
                                       "Rock-Ice","Cloud", "Burn-Forest", 
                                       "Burn-Grassland", "Burn-Shrub", "Alpine Herb",
                                       "Alpine Shrub"))
levels(cov_rasters$landcov) <- att_tbl

# need to tabulate out every habitat type to separate columns
# can do this with glm(used ~ I(habitat_type))
